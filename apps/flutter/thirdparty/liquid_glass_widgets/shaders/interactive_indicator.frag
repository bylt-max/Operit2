...
uniform vec4 uData8; // 32..35 (blur, unused, unused, unused)
...
void main() {
  ...
  float thicknessFactor = clamp(uThickness / 40.0, 0.05, 6.0);
  
  // Keep refraction tied to a compact control height. Chat bubbles can grow
  // vertically with content, but the glass material should not become thicker.
  float refractionHeight = min(uSize.y, 44.0);
  vec2 edgeOffsetLogical = surfaceNormal * edgeInfluence * bendStrength * thicknessFactor * refractionHeight * 0.35;
  ...
  vec2 distort = surfaceNormal * edgeInfluence * uChromaticAberration * thicknessFactor;
  ...
  float keyHighlight = pow(max(edgeLightCatch, 0.0), 8.0) * uLightIntensity * 0.5 * thicknessFactor;
  ...
  float kickHighlight = pow(max(-edgeLightCatch, 0.0), 12.0) * uLightIntensity * 0.5 * thicknessFactor;
  ...
  finalColor += vec3(1.0) * fresnel * uGlowIntensity * thicknessFactor; // TUNE via LiquidGlassSettings.glowIntensity
  ...
  // Respect the caller-provided multiplier even when a background capture exists;
  // chat bubbles grow with content, so a hardcoded body alpha makes tall bubbles
  // read as an increasingly thick slab.
  float standardBaseAlpha = uBaseAlphaMultiplier * mix(0.6, 1.0, uInteractionIntensity);
  float baseAlpha = standardBaseAlpha * thicknessFactor;
  ...
  float edgeAlpha = standardEdgeAlpha * thicknessFactor;
  ...
  float blurReveal = smoothstep(0.05, 1.2, uBlur);
  float refractionMask = max(edgeInfluence, borderMask);
  float blurredInteriorAlpha = mix(glassAlpha * 0.18, glassAlpha, refractionMask);
  glassAlpha = mix(glassAlpha, blurredInteriorAlpha, blurReveal);
